# ===== NEW SUB-TAB: AGRICULTURAL ECONOMICS APPLICATIONS =====
# Insert this as a new sub-tab in tab_regression (after CLRM, before simple regression)

with subtab_economics:
    st.subheader("📊 Aplikasi Ekonomi Pertanian")
    
    st.markdown("""
    ## 🌾 ANALISIS EKONOMI PERTANIAN DENGAN REGRESI
    
    Tab ini menyediakan **tools interaktif** untuk analisis ekonomi pertanian yang umum digunakan dalam penelitian dan praktik.
    """)
    ...
    econ_tool = st.radio(
        ...
        ["🏭 Production Function (Fungsi Produksi)",
         "📈 Elasticity Calculator (Kalkulator Elastisitas)",
         "💰 Cost & Profit Analysis (Analisis Biaya & Keuntungan)",
         "🎯 Optimal Input Calculator (Kalkulator Input Optimal)"],
        horizontal=False
    )
    
    # ===== TOOL 1: PRODUCTION FUNCTION =====
    if econ_tool == "🏭 Production Function (Fungsi Produksi)":
        st.divider()
        st.subheader("🏭 Cobb-Douglas Production Function")
        ...
        **Average Product:**
        ...
        if df_prod is not None:
            ...
            with col_spec1:
                output_var = st.selectbox("Output Variable (Y):", df_prod.columns, index=1)
    ...
    # ===== TOOL 2: ELASTICITY CALCULATOR =====
    elif econ_tool == "📈 Elasticity Calculator (Kalkulator Elastisitas)":
        st.divider()
        st.subheader("📈 Price Elasticity of Demand & Supply")
