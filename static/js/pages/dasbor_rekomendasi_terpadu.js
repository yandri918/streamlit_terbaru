...
// New robust implementation for Integrated Recommendation
async function submitIntegratedRecommendation(event) {
    event.preventDefault();
    logToPage('📝 Integrated Recommendation: submitting...');
    const resultDiv = document.getElementById('result-integrated');
    ...
    try {
        const resp = await fetch(`${baseUrl}${apiPrefix}/recommendation/integrated`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload),
        });
        logToPage(`Response status: ${resp.status}`);
        const data = await resp.json();

        if (data.success) {
            const r = data.data;
            let html = `
                <h3>🌱 Hasil Rekomendasi Terpadu</h3>
                <h4>🌾 Bibit & Varietas</h4>
                <p><strong>Kriteria:</strong> ${r.bibit.kriteria}</p>
                <p><strong>Contoh Varietas:</strong> ${r.bibit.rekomendasi.join(', ')}</p>
                <p><em>Tips:</em> ${r.bibit.tips}</p>

                <h4>🧪 Pemupukan (${r.pemupukan.fase})</h4>
                <ul>
                    <li><strong>Rasio NPK:</strong> ${r.pemupukan.rasio_npk}</li>
                    <li><strong>Aplikasi Tanah:</strong> ${r.pemupukan.aplikasi_tanah}</li>
                    <li><strong>Aplikasi Daun:</strong> ${r.pemupukan.aplikasi_daun}</li>
                </ul>

                <h4>🛡️ Pengendalian Hama & Penyakit</h4>
`;
            if (r.pengendalian) {
                if (typeof r.pengendalian === 'string') {
                    html += `<p>${r.pengendalian}</p>`;
                } else {
                    const p = r.pengendalian;
                    html += `<h5>${p.strategy.name}</h5>`;
                    html += `<p>${p.strategy.description}</p>`;
        ...
        console.error('Integrated fetch error:', e);
        logToPage(`Error: ${e.message}`);
        resultDiv.innerHTML = `<p style="color:red;"><strong>Error:</strong> Gagal terhubung ke server.</p>`;
    }
}
...
function initDashboard() {
    ...
    // --- Data Integration: Load saved soil data ---
    try {
        const savedData = localStorage.getItem('agrisensa_soil_data');
        if (savedData) {
            const data = JSON.parse(savedData);
            console.log('🔄 Dashboard: Loading saved soil data:', data);

            // Pre-fill Modul 14 (Crop Recommendation)
            if (data.n) {
                const cropN = document.getElementById('crop-n-input');
                if (cropN) cropN.value = data.n;
            }
            if (data.p) {
                const cropP = document.getElementById('crop-p-input');
                if (cropP) cropP.value = data.p;
            }
            if (data.k) {
                const cropK = document.getElementById('crop-k-input');
                if (cropK) cropK.value = data.k;
            }
            if (data.ph) {
                const cropPh = document.getElementById('crop-ph-input');
                if (cropPh) cropPh.value = data.ph;
            }

            // Pre-fill Modul 15 (Yield Prediction)
            if (data.n) {
                const yieldN = document.getElementById('yield-n-input');
                if (yieldN) yieldN.value = data.n;
            }
            if (data.p) {
                const yieldP = document.getElementById('yield-p-input');
                if (yieldP) yieldP.value = data.p;
            }
            if (data.k) {
                const yieldK = document.getElementById('yield-k-input');
                if (yieldK) yieldK.value = data.k;
            }
            if (data.ph) {
                const yieldPh = document.getElementById('yield-ph-input');
                if (yieldPh) yieldPh.value = data.ph;
            }
        }
    } catch (e) {
        console.error('Error loading saved data on dashboard:', e);
    }

    // --- Modul 9: Dasbor Rekomendasi Terpadu ---
    const integratedForm = document.getElementById('integrated-recommendation-form');
    if (integratedForm) {
        integratedForm.addEventListener('submit', submitIntegratedRecommendation);
        integratedForm.style.borderLeft = '5px solid #4caf50'; // visual confirmation
        logToPage("✅ Modul 9 Form Found & Listener Attached");
    }
