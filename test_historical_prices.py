...
def test_historical_prices():
    print("Testing /get-historical-prices endpoint for all commodities...")
    all_passed = True
    
    for commodity in commodities:
        try:
            response = requests.post(
                f"{BASE_URL}/get-historical-prices",
                json={"commodity": commodity, "range": 30},
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get("success"):
                    print(f"✅ {commodity}: OK ({len(data['prices'])} data points)")
                else:
